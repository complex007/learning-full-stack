...
var emitToAllParticipants = function( io, eventName , participants){
    for ( let sId = 0; sId < participants.length; sId++){
        console.log("send ",eventName," to client for all participants");
        io.to(participants[sId]).emit(eventName,participants);             
    }     
}
...
module.exports=function(io){
    ...
    io.on('connection',(socket) => {
        ...
        if( sessionId in collaborations){
            console.log("working on this sessionId now");
            collaborations[sessionId].participants.push(socket.id);  
            emitToAllParticipants(io, 'collaboration-info',collaborations[sessionId].participants);             
        }
        else{
        ...
        socket.on('disconnect',()=>{
            ...
            if ( sessionId in collaborations){
                ...
                if (index>=0){
                    ...
                    // if no participant is working on this sessionId,
                    // deactiving the redis
                    if(participants.length == 0){
                        console.log(`last participant in collaborations,committing to redis and remove from memory`);
